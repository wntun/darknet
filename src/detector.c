    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
	...
	image_count += 1;
        ...
        network_predict(net, X);
	total_time += ((double)get_time_point()-time);
    ...
    //double duration = ((double)get_time_point()-start)/(1000*image_count);
    double avg_detection = total_time/(image_count*1000.0);
    double fps = 1000/avg_detection;
    printf("\nNumber of images %d. \nAverage detection time %lf milliseconds and frame-per-second %lf.\n", image_count,avg_detection, fps);
